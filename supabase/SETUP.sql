...
-- ============ Phase 3, Slice 3: flags, login activity, hidden Sir notes ============
-- (Same as migrations/009_phase3_activity_notes_flags.sql — safe to re-run.)
...
create index if not exists login_activity_student_idx on login_activity (student_id, created_at desc);
alter table login_activity enable row level security;
do $$ begin
  if not exists (select 1 from pg_policies where tablename = 'login_activity' and policyname = 'login_activity: admin read') then
    create policy "login_activity: admin read" on login_activity for select using (is_admin());
  end if;
end $$;
grant select, insert on login_activity to authenticated;
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename = 'task_sir_notes' and policyname = 'task_sir_notes: admin all') then
    create policy "task_sir_notes: admin all" on task_sir_notes for all using (is_admin()) with check (is_admin());
  end if;
end $$;
...
-- ============ Phase 4, Slice 4: task-scoped real-time chat ============
-- (Same as migrations/010_phase4_task_chat.sql — safe to re-run.)
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename = 'task_messages' and policyname = 'task_messages: read own') then
    create policy "task_messages: read own" on task_messages
      for select using (
        is_admin() or exists (select 1 from tasks t where t.id = task_messages.task_id and t.student_id = auth.uid())
      );
  end if;
end $$;
do $$ begin
  if not exists (select 1 from pg_policies where tablename = 'task_messages' and policyname = 'task_messages: insert own') then
    create policy "task_messages: insert own" on task_messages
      for insert with check (
        sender_id = auth.uid() and (
          (is_admin() and sender_role = 'admin')
          or (sender_role = 'student' and exists (select 1 from tasks t where t.id = task_messages.task_id and t.student_id = auth.uid()))
        )
      );
  end if;
end $$;
...
do $$ begin
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime')
     and not exists (select 1 from pg_publication_tables where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'task_messages') then
